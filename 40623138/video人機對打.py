...
def getballposition():
    ...
    errorCode,position_S=vrep.simxGetObjectPosition(clientID,Sphere_handle,-1,vrep.simx_opmode_oneshot)      #指定球的位置代號
    A1v=position_S[1] - position_A1R[1]　　　　　　　#設定一個代號為A１v 會等於 球的Y軸的位置-桿子的Y軸位置
    AA1v=position_S[0] - position_A1R[0]           #設定一個代號為AA１v 會等於 球的X軸的位置-桿子的X軸位置
    while True:
        if A1v <= 0 and AA1v <= 0.015:　　　　　　　　　　　　　　　　　　　#當Y軸的距離<=０, X軸的距離<=０.０１５　會執行下列五行程式
        ...
        elif A1v > 0 and AA1v <= 0.015:　　　　　　　　　　　　　　　　　　#當Y軸的距離>０ , X軸的距離<=０.０１５
            errorCode,position_A1R=vrep.simxGetObjectPosition(clientID,A1_handle,-1,vrep.simx_opmode_oneshot)
            errorCode,position_S=vrep.simxGetObjectPosition(clientID,Sphere_handle,-1,vrep.simx_opmode_oneshot)
            A1v =position_S[1]- position_A1R[1]
            AA1v =position_S[0] - position_A1R[0]
            vrep.simxSetJointTargetVelocity(clientID,R1_handle,R_KickBallVel,vrep.simx_opmode_oneshot_wait)　　　#給R１旋轉軸 順時針旋轉
            ...
            A1v =position_S[1]- position_A1R[1]
            AA1v =position_S[0] - position_A1R[0]
            vrep.simxSetJointTargetVelocity(clientID,R1_handle,B_KickBallVel,vrep.simx_opmode_oneshot_wait)　　　#給R１旋轉軸 逆時針旋轉
            ...
            #以下就是鍵盤的操控程式
        try:
            if keyboard.is_pressed('w'): 　　　　#設定鍵盤W鍵
                vrep.simxSetJointTargetVelocity(clientID,R2_handle,R_KickBallVel,vrep.simx_opmode_oneshot_wait)　　#控制R２旋轉軸可以順時針移動
            elif keyboard.is_pressed('x'):  　　#設定鍵盤X鍵
                vrep.simxSetJointTargetVelocity(clientID,R2_handle,B_KickBallVel,vrep.simx_opmode_oneshot_wait)　　#控制R２旋轉軸可以逆時針移動
            elif keyboard.is_pressed('d'):  　　#設定鍵盤D鍵
                vrep.simxSetJointTargetVelocity(clientID,P2_handle,0.05,vrep.simx_opmode_oneshot_wait)　　　　　　　　　#控制P２平移軸可以以０.０５速度移動
            elif keyboard.is_pressed('s'):　　　#設定鍵盤S鍵
                vrep.simxSetJointTargetVelocity(clientID,P2_handle,0,vrep.simx_opmode_oneshot_wait)
                vrep.simxSetJointTargetVelocity(clientID,R2_handle,0,vrep.simx_opmode_oneshot_wait)　　　　　　　　　　　 #控制P２平移軸與R２旋轉軸速度為０
            elif keyboard.is_pressed('a'):  　　#設定鍵盤A鍵
                vrep.simxSetJointTargetVelocity(clientID,P2_handle,-0.05,vrep.simx_opmode_oneshot_wait)　　　　　　　　 #控制P２平移軸可以以-０.０５速度移動
            else:
                pass
        except:
            break
